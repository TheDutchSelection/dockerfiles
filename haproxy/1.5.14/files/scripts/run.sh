      ...
      if [[ "$haproxy_backends" == *"$app"* ]]; then
        if [[ ! -z "$frontend_domain_redirects" ]]; then
          ...
          do
            ...
            local redirect_list="$redirect_list"$'\n'"  redirect prefix http://$destination code 301 if { hdr(host) -m beg -i $source }"
          done
        fi

        if [[ "$frontend_domain_force_ssl" == "1" && "$https_filters" != "1" ]]; then
          local redirect_list="$redirect_list"$'\n'"  redirect scheme https code 301 if { hdr(host) -m beg -i $domains } !{ ssl_fc }"
        else
          local acl_list="$acl_list"$'\n'"  acl $acl_name hdr(host) -m sub -i $domains"
