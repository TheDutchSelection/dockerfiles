...
result=$(curl 'http://'"$CAPTAIN_HOST_PUBLIC_IP"':'"$CAPTAIN_HOST_PORT"'/update/?env='"$APP_ENV"'&app='"$APP_ID"'&docker_image_name='"$DOCKER_IMAGE_NAME"'&docker_image_tag='"$DOCKER_IMAGE_TAG"'&probe_path='"$PROBE_PATH"'&gracefully_kill_workers='"$GRACEFULLY_KILL_WORKERS")
