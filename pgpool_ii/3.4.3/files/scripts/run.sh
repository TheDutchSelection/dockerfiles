...
cp -p /usr/local/bin/copy_base_backup.sh "$BACKEND_DATA_DIRECTORY"
cp -p /usr/local/bin/pgpool_recovery_pitr.sh "$BACKEND_DATA_DIRECTORY"
cp -p /usr/local/bin/pgpool_remote_start "$BACKEND_DATA_DIRECTORY"
