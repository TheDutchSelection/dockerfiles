...
init_data_directory_and_create_superuser() {
  ...
  if [[ ! $(ls -A "$DATA_DIRECTORY") ]]; then
    ...
    cp -R /var/lib/postgresql/9.3/main/* "$DATA_DIRECTORY"
...
cp -p /etc/postgresql/9.3/main/postgresql_template.conf /etc/postgresql/9.3/main/postgresql.conf

escaped_data_directory=${DATA_DIRECTORY//\//\\\/}

echo "set values to postgresql.conf file..."
sed -i "s/##data_directory##/$escaped_data_directory/g" /etc/postgresql/9.3/main/postgresql.conf
...
/usr/lib/postgresql/9.3/bin/postgres -c config_file=/etc/postgresql/9.3/main/postgresql.conf &
