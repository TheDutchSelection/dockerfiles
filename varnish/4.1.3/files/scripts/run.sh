...
read -r -d '' varnish_vcl_backend_response_base << EOM || true
...
create_vcl_backend_response () {
  ...
  local varnish_vcl_backend_response="$varnish_vcl_backend_response_base"
  ...
  if [[ -z "$ALLOWED_301_CACHING_FILES" ]]; then
    ...
    local allowed_301_caching_conditions="(beresp.status == 301)"
    local varnish_vcl_backend_response="$varnish_vcl_backend_response"$'\n'$'\n'"  if ("
    for file in $ALLOWED_301_CACHING_FILES
    do
      if [[ "$first_file" = true ]]; then
